...
export class StrengthValuesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(this.strengthValues$.pipe(
      tap(strengthValues => {
        if (strengthValues) {
  ...
  getWeight(index: number) {
    return this.strengthValuesForms.controls[index].get('weight') as FormControl;
  }
  ...
  private subscribeWeigthChages(formGroup: FormGroup) {
    this.subscription.add(
      formGroup.get('weight').valueChanges.pipe(
        tap(weight => {
          if (weight) {
            const reps = formGroup.get('reps').value;
            formGroup.patchValue({ oneRepMax: this.calcOneRepMax(weight, reps) });
            this.updateTotal();
          }
        })
      ).subscribe()
  ...
  private subscribeRepsChages(formGroup: FormGroup) {
    this.subscription.add(
      formGroup.get('reps').valueChanges.pipe(
        tap(reps => {
